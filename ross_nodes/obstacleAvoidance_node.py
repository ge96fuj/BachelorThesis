...
def obstacleAvoidance_node():
    ...
    while not rospy.is_shutdown():
        ...
        for obstacle in obstacles.copy():
            for obstacle in obstacles.copy():
                #SKIP IF ITS TRAFFIC LIGHT , DONT AVOID 
                 if "traffic_light" in obstacle:
                     continue  
            ...
            if distance < THRESHOLD and closest_lane and closest_lane == current_lane:
                print(f"Obstacle {obstacle} in pose: {obstacles[obstacle]} Lane: {closest_lane}")
                print(f"\nFound {obstacle} in pose: {obstacles[obstacle]}, Lane: {closest_lane} distance: {distance} treshold {THRESHOLD}")
